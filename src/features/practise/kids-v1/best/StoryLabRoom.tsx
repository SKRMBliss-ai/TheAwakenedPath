  ...
  return <motion.main initial={still ? false : { opacity: 0, scale: .98 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: .7 }} className={`sl-room ${still ? 'sl-still' : ''} ${quiet ? 'sl-quiet' : ''}`} style={{ fontFamily: FONT }} data-step={step}>
    ...
    <div className="sl-layout">
      ...
      <section className="sl-theatre" aria-labelledby="sl-prompt">
        ...
        <motion.div key={step} className="sl-stage" initial={still ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .5 }}>
          {step === 2 && <div className="sl-thought-field"><div className="sl-thought-clouds">{options.map((text, i) => <motion.button key={text} className="sl-thought-cloud" style={{ '--drift-delay': `${i * -.7}s` } as CSSProperties} whileHover={still ? undefined : { scale: 1.06, y: -5 }} onClick={() => capture(text)}>{text}</motion.button>)}</div><img className="sl-thinking-boy" src="/mind-gym/story-lab/thinking-boy-clean.webp" alt="Your explorer thinking" /><div className="sl-desk" aria-hidden="true" /></div>}
          {step === 3 && <><figure className="sl-memory-theatre" aria-label="Memory theatre"><div className="sl-film-perforations" /><img src="/mind-gym/story-lab/memory-illustration.webp" alt="An illustrated example of a moment in a school playground" /><figcaption>Picture your own moment — this is just an example.</figcaption><div className="sl-film-perforations" /></figure><div className="sl-event-options">{options.map((text, i) => <button key={text} onClick={() => capture(text)}><span aria-hidden="true">{['☏', '◷', '♧', '✧', '↝', '…'][i]}</span>{text}</button>)}</div></>}
          {step === 4 && <div className="sl-assembly">
            <div className="sl-orbit" aria-label="Your feeling, body, thought and event joining together"><svg className="sl-connections" viewBox="0 0 560 300" aria-hidden="true"><path d="M90 70 Q280 0 470 70 Q540 150 420 230 Q280 290 140 230 Q20 150 90 70 M90 70 Q280 170 420 230 M470 70 Q280 170 140 230" /></svg>{[carried.feeling, carried.body?.join(', '), thought, event].map((text, i) => <motion.div key={i} className={`sl-orbit-clue sl-orbit-${i}`} initial={still ? false : { opacity: 0, scale: .6 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: still ? 0 : .8, delay: still ? 0 : i * .18 }}><b>{STEPS[i]}</b><span>{text || 'Not sure yet'}</span></motion.div>)}<div className="sl-story-orb" aria-hidden="true"><img src="/mind-gym/story-lab/thinking-boy-clean.webp" alt="" /></div></div>
          ...
          {(step === 5 || step === 6) && <><p className="sl-same-event"><b>The same event</b> {event}</p><div className="sl-possibility-windows"><div className="sl-window sl-original"><h3>Original story</h3><img src="/mind-gym/story-lab/thinking-boy-clean.webp" alt="" /><p>“{story}”</p></div><div className="sl-window sl-another"><h3>Another possibility</h3>{alternative ? <img src="/mind-gym/story-lab/thinking-boy-clean.webp" alt="" /> : <span className="sl-possibility-light" aria-hidden="true">✧</span>}<p>{alternative ? `“${alternative}”` : 'A little space for another way to see it…'}</p></div></div>
