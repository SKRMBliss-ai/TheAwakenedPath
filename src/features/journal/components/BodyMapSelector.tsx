...
export default function BodyMapSelector({ onSelect, selectedArea }: BodyMapSelectorProps) {
    const handleSelect = (zone: any) => {
        if (onSelect) {
            onSelect(selectedArea?.id === zone.id ? null : zone);
        }
    };
                ...
                <div className="relative flex justify-center">
                    ...
                    >
                        ...
                        {BODY_ZONES.map((zone, i) => {
                            ...
                            const isOther = selectedArea && selectedArea.id !== zone.id;
                            ...
                            return (
                                <g key={zone.id}>
                                    ...
                                    {/* Pulsing outer ring (idle) */}
                                    {!isSelected && !isOther && (
                                        <circle cx={zone.cx} cy={zone.cy} r={14} fill="none" stroke={zone.color} strokeWidth="1.5" opacity="0.4">
                                            <animate attributeName="r" values="14;20;14" dur={`${3 + i * 0.3}s`} repeatCount="indefinite" />
                                            <animate attributeName="opacity" values="0.4;0.1;0.4" dur={`${3 + i * 0.3}s`} repeatCount="indefinite" />
                                        </circle>
                                    )}
